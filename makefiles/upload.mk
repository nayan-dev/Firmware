...
ifeq ($(SYSTYPE),Linux)
SERIAL_PORTS		?= "/dev/serial/by-id/*3D_Robotics*,/dev/serial/by-id/*_PX4_*,/dev/serial/by-id/*Bitcraze*,/dev/serial/by-id/*Autopilot*"
endif
ifeq ($(SERIAL_PORTS),)
...
upload-serial-px4fmu-v5:	$(BUNDLE) $(UPLOADER)
	$(Q) $(PYTHON) -u $(UPLOADER) --port $(SERIAL_PORTS) $(BUNDLE)
	
upload-serial-px4fmu-v6:	$(BUNDLE) $(UPLOADER)
	$(Q) $(PYTHON) -u $(UPLOADER) --port $(SERIAL_PORTS) $(BUNDLE)		

upload-serial-px4fmu-v3:	$(BUNDLE) $(UPLOADER)
	$(Q) $(PYTHON) -u $(UPLOADER) --port $(SERIAL_PORTS) $(BUNDLE)

upload-serial-px4fmu-v4:	$(BUNDLE) $(UPLOADER)
	$(Q) $(PYTHON) -u $(UPLOADER) --port $(SERIAL_PORTS) $(BUNDLE)
